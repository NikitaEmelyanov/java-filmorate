...
public record RequestFilmDto(
    ...
    @NotEmpty(message = "Название не может быть пустым", groups = {FilmCreateValidation.class,
        FilmUpdateValidation.class})
    @Length(max = 200, message = "максимальная длина описания — 200 символов", groups = {
        FilmCreateValidation.class, FilmUpdateValidation.class})
    String name,

    @Length(max = 200, message = "Максимальная длинна описания 200 символов", groups = {
        FilmCreateValidation.class, FilmUpdateValidation.class})
    String description,
    ...
    @Positive(message = "Продолжительность должна быть положительным числом", groups = {
        FilmCreateValidation.class, FilmUpdateValidation.class})
    int duration
) {
